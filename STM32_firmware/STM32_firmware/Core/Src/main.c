...
uint16_t pressure = 0;
float voltage =0;
float voltage_filt=0;
int Timer_val=0;
float peso;
volatile uint8_t usb_send_flag = 0;
char usb_msg_buffer[128];  // You can double-buffer if needed

//char msg[20];

// coefficients

float fir_coefficients[200];
float buffer_fir[200];
int16_t counter_fir;
float fir_out;
int index_fir;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (usb_send_flag) {
            ...
            // Else: USBD_BUSY → retry on next loop
        }
